...
export default function App() {
  ...
  return <div ref={revealRef}>
    ...
    <a href={`https://wa.me/${whatsapp}?text=Hola%20Jacqueline%2C%20quisiera%20cotizar%20un%20tatuaje%20en%20Night%20Fury%20Tattoo.`} target="_blank" rel="noreferrer" className="floating-wa-btn" aria-label="Cotizar por WhatsApp">◔</a>
...
function QuoteWizard({ step, setStep, onClose, whatsapp }) {
  ...
  const send = event => { event.preventDefault(); if (!form.name || !form.idea) return; const message = `Hola Jacqueline, soy ${form.name}. Quisiera cotizar: ${form.idea}. Estilo: ${form.style}. Zona: ${form.placement}. Tamaño: ${form.size}. Tinta: ${form.color}.`; window.open(`https://wa.me/${whatsapp}?text=${encodeURIComponent(message)}`, '_blank'); onClose(); };
