...
const works = [
  ...
  ['fineline', 'Spider Lily & Japanese Fan', 'Fine Line / Red Accent', 'Abanico japonés en costillas con flores de lis de fuego en tinta carmín y cordones de trazo fino.', 'red-fan-floral.png', 'Red Spider Lily & Fan'],
  ['microrealismo', 'Surrealist Eyes & Latin Script', 'Microrealismo', "Mirada microrealista con frase clásica 'AMOR VINCIT OMNIA' en antebrazo. Sombreado delicado.", 'eyes-script.png', 'Surrealist Eyes & Script'],
  ['ilustrativo', 'Botanical Peony Arm Flow', 'Ilustrativo', 'Composición floral de peonías y hojas con flujo anatómico en hombro y antebrazo. Trazo dinámico.', 'peony-shoulder.png', 'Botanical Peony Flow'],
  ['fineline', 'Delicate Script & Geometry', 'Fine Line', 'Caligrafía fina de alta definición con aguja de calibre quirúrgico. Pieza protegida en cicatrización.', 'fine-line.png', 'Delicate Custom Lettering'],
  ['blackwork', 'Dark Dragon Wing Scale', 'Blackwork', 'Textura de escamas y gradientes de tinta negra con técnica de stippling.', 'blackwork.png', 'Dragon Scale & Shadows']
];

const services = [
  ['Fine Line & Micro', 'Líneas ultrasutiles, caligrafía limpia y detalles de precisión microscópica en tinta negra o gris.'],
  ['Blackwork & Dark', 'Sombra intensa, texturas de escama, alas y piezas con fuerte carácter y marcado contraste.'],
  ['Red Ink Accents', 'Toques de tinta carmín en detalles florales, símbolos o elementos de composición para resaltar el diseño.'],
  ['Coberturas & Retoques', 'Evaluación de piezas antiguas o retoques para revitalizar líneas e intensidad de color.']
];
...
const faqs = [
  ['¿Cómo solicito una cotización?', 'Puedes dar clic en el botón "Cotizar mi tatuaje" de esta página para llenar el formulario rápido o enviarnos un mensaje directo a WhatsApp con tus fotos de referencia, zona del cuerpo y tamaño aproximado en centímetros.'],
  ['¿Se requiere anticipo para agendar?', 'Sí, para apartar tu fecha en la agenda y ponernos a trabajar en la propuesta de tu diseño se requiere un anticipo. Este monto se descuenta del costo total el día de tu cita.'],
  ['¿El diseño será totalmente exclusivo?', 'Absolutamente. En Night Fury Tattoo adaptamos tus referencias para crear una pieza única adaptada a tu cuerpo y estilo personal. Nunca copiamos tatuajes de otros artistas tal cual.'],
  ['¿Tatuas a menores de edad?', 'Únicamente se realizan trabajos a personas mayores de 18 años identificadas con identificación oficial vigente (INE/Pasaporte).']
];

const Arrow = () => <span className="btn-icon-wrapper">→</span>;
...
export default function App() {
  ...
  return <div ref={revealRef}>
    ...
    <main>
      <section id="hero" className="hero-section"><div className="hero-bg-media"><video autoPlay muted loop playsInline aria-hidden="true"><source src="/videos/fondo.mp4" type="video/mp4" /></video></div><div className="hero-watermark">NF</div><div className="container hero-content"><div className="eyebrow">01 · ESTUDIO PRIVADO EN DURANGO</div><h1 className="hero-headline">Tu historia merece <span>dejar marca.</span></h1><p className="hero-lead">Una propuesta oscura, delicada e intensa que transforma tus ideas y emociones en piezas únicas. Creadas con técnica de alta precisión, cuidado sanitario y actitud por <strong>Jacqueline</strong>.</p><div className="hero-actions"><button className="btn btn-primary" onClick={openQuote}>Cotizar mi tatuaje <Arrow /></button><a href="#portafolio" className="btn btn-secondary">Ver trabajos seleccionados</a></div><div className="hero-meta"><span className="pill-badge">Durango, Dgo. México</span><a href="https://instagram.com/night.fury_ttt" target="_blank" rel="noreferrer" className="pill-badge">@night.fury_ttt</a><span className="pill-badge">Atención exclusiva por cita</span></div></div></section>
      <div className="section-divider" />

      <section id="portafolio" className="section"><div className="container"><div className="portfolio-header"><div data-reveal><div className="eyebrow">02 · PORTAFOLIO</div><h2>Trabajos recientemente ejecutados</h2></div><ul className="filter-bar">{[['all', 'Todos'], ['blackwork', 'Blackwork'], ['fineline', 'Fine Line'], ['microrealismo', 'Microrealismo'], ['ilustrativo', 'Ilustrativo']].map(([value, label]) => <li key={value}><button className={`filter-btn ${filter === value ? 'active' : ''}`} onClick={() => setFilter(value)}>{label}</button></li>)}</ul></div><div className="portfolio-grid">{works.map(work => <article key={work[1]} data-reveal className={`portfolio-item ${filter !== 'all' && filter !== work[0] ? 'is-filtered-out' : ''}`} style={{ display: filter !== 'all' && filter !== work[0] ? 'none' : undefined }} onClick={() => setSelectedWork(work)}><img src={`/img/${work[4]}`} alt={work[1]} /><div className="portfolio-overlay"><span className="portfolio-tag">{work[2]}</span><h3 className="portfolio-title">{work[5]}</h3></div></article>)}</div></div></section>
      <div className="section-divider" />

      <section id="artista" className="section"><div className="container about-grid"><div className="about-image-card" data-reveal><img src="/img/artist-studio.png" alt="Jacqueline Tattoo Artist Studio" /><div className="artist-badge"><div><strong>Jacqueline</strong><span>Tatuadora & Creadora de Night Fury</span></div><a href="https://instagram.com/jacqueline_bdz" target="_blank" rel="noreferrer" className="btn btn-secondary">@jacqueline_bdz</a></div></div><div data-reveal><div className="eyebrow">03 · LA ARTISTA</div><h2>Sensibilidad artística con actitud propia</h2><p>Crear un tatuaje es un proceso de confianza mutua. Mi compromiso en <strong>Night Fury Tattoo</strong> es ofrecerte un espacio seguro, limpio y libre de juicios donde tu referencia o idea abstracta se traduzca en una pieza limpia y perdurable.</p><br /><p>Me especializo en trazos delicados, composiciones oscuras, contraste de tinta roja y piezas con significado personal.</p><div className="artist-stats"><div><strong>100%</strong><span>Diseños adaptados al cuerpo</span></div><div><strong>Durango</strong><span>Atención por cita previa</span></div></div></div></div></section>
      <div className="section-divider" />

      <section id="estilos" className="section"><div className="container"><div className="eyebrow" data-reveal>04 · SERVICIOS</div><h2 data-reveal>Estilos y especialidades</h2><p>Piezas personalizadas diseñadas partiendo de tu idea o referencia visual.</p><div className="services-grid">{services.map(([title, text]) => <div className="double-bezel" data-reveal key={title}><div className="double-bezel-inner"><div className="service-mark">✦</div><h3>{title}</h3><p>{text}</p></div></div>)}</div></div></section>
      <div className="section-divider" />
      ...
      <section id="higiene" className="section"><div className="container"><div className="hygiene-grid"><div data-reveal><div className="eyebrow">06 · SEGURIDAD SANITARIA</div><h2>Tu salud e higiene son prioridad absoluta</h2><p>Trabajamos bajo estándares clínicos rigurosos para garantizar que tu sesión sea completamente segura.</p><ul className="hygiene-list">{['Agujas y consumibles 100% esterilizados y desechables.', 'Pigmentos veganos homologados sin metales pesados.', 'Sanitización previa y barreras de protección grado médico.'].map(text => <li key={text}><Check />{text}</li>)}</ul></div><div className="hygiene-image" data-reveal><img src="/img/hygiene-equipment.png" alt="Equipo de tatuaje estéril" /></div></div><div className="faq-heading"><h2>Preguntas Frecuentes</h2><p>Resolvemos tus dudas antes de agendar tu cita.</p></div><div className="faq-accordion">{faqs.map(([question, answer], index) => <div className={`faq-item ${faqOpen === index ? 'active' : ''}`} data-reveal key={question}><button className="faq-header" onClick={() => setFaqOpen(faqOpen === index ? null : index)} aria-expanded={faqOpen === index}>{question}<span>⌄</span></button><div className="faq-content"><p>{answer}</p></div></div>)}</div></div></section>

      <section id="contacto" className="section contact-section"><div className="container" data-reveal><div className="eyebrow">07 · COTIZACIÓN DIRECTA</div><h2>¿Listo para darle vida a tu próximo tatuaje?</h2><p>Cuéntanos tu idea. Sin presiones y con respuesta rápida para resolver todas tus dudas sobre diseño, costo y agenda.</p><div className="contact-actions"><button className="btn btn-primary" onClick={openQuote}>Iniciar Cotización Ahora <Arrow /></button><a href="https://instagram.com/night.fury_ttt" target="_blank" rel="noreferrer" className="btn btn-secondary">Ver Instagram @night.fury_ttt</a></div></div></section>
    </main>
    <footer><div className="container">Night Fury Tattoo · Estudio Privado de Tatuajes en Durango, Dgo. México · Arte por Jacqueline<br />Servicios prestados bajo estricto protocolo de asepsia. Exclusivo para mayores de 18 años.</div></footer>
    <a href="https://wa.me/526180000000?text=Hola%20Jacqueline%2C%20quisiera%20cotizar%20un%20tatuaje%20en%20Night%20Fury%20Tattoo." target="_blank" rel="noreferrer" className="floating-wa-btn" aria-label="Cotizar por WhatsApp">◔</a>
    {selectedWork && <Lightbox work={selectedWork} onClose={() => setSelectedWork(null)} onQuote={openQuote} />}
    {quoteOpen && <QuoteWizard step={step} setStep={setStep} onClose={closeOverlays} />}
  </div>;
}

function Lightbox({ work, onClose, onQuote }) { return <div className="lightbox-modal active" onClick={event => event.target === event.currentTarget && onClose()}><div className="lightbox-content"><button className="lightbox-close" onClick={onClose} aria-label="Cerrar">×</button><div className="lightbox-image-wrap"><img src={`/img/${work[4]}`} alt={work[1]} /></div><div className="lightbox-info"><div><span className="portfolio-tag">{work[2]}</span><h3>{work[1]}</h3><p>{work[3]}</p></div><button className="btn btn-primary" onClick={() => { onClose(); onQuote(); }}>Cotizar pieza similar</button></div></div></div>; }

function QuoteWizard({ step, setStep, onClose }) {
  ...
  const send = event => { event.preventDefault(); if (!form.name || !form.idea) return; const message = `Hola Jacqueline, soy ${form.name}. Quisiera cotizar: ${form.idea}. Estilo: ${form.style}. Zona: ${form.placement}. Tamaño: ${form.size}. Tinta: ${form.color}.`; window.open(`https://wa.me/526180000000?text=${encodeURIComponent(message)}`, '_blank'); onClose(); };
